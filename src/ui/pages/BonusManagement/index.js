...
const BonusManagement = () => {
    ...
    const columns = [
        ...
        { name: "Contact Info", wrap: true, selector: row => row?.emailId || row?.mobileNumber, },
        { name: "Description", sortable: true, wrap: true, selector: row => row.description, },
        { name: "Amount", sortable: true, wrap: true, selector: row => `${row?.amount} ${row?.short_name}`, },
    ...
    const handleFundDeposit = async () => {
        ...
        await AuthService.completeBonusRequest().then(async result => {
            LoaderHelper.loaderStatus(false);
            if (result?.success) {
                try {
                    let filteredData = result?.data?.map((item, index) => ({ ...item, index: index + 1 }))
                    setFundsDeposit(filteredData?.reverse());
    ...
    function searchObjects(e) {
        const keysToSearch = ["uuid", "currency", "emailId", "firstName", "from_address", "to_address", "transaction_hash", "amount", "mobileNumber", "user_id"];
